...
export const api = {
  ...
  addIocs: (
    ...
    request<{ iocs: Ioc[]; added: number; duplicates: string[] }>(`/api/hunts/${huntId}/iocs`, {
      method: 'POST',
  ...
  continueHunt: (huntId: string, budgets?: { max_iterations?: number; max_siem_queries?: number }) =>
    request<{ hunt_id: string; status: HuntStatus; running: boolean }>(
      `/api/hunts/${huntId}/continue`,
      { method: 'POST', body: JSON.stringify(budgets ?? {}) },
    ),

  resumeHunt: (huntId: string, payload: { instruction?: string; from_query_id?: string }) =>
