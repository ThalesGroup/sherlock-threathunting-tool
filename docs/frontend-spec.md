...
The screen also carries resumption. For an interrupted hunt whose loop state has been saved, "Continue where it left off" relaunches the same investigation with its transcript, its pseudonyms and its remaining budgets (if exhausted, the budget checkpoint asks again in the feed). Otherwise — or in addition — a "Resume this hunt" card (free-form question or instruction, choice of the query after which to restart) creates a new linked investigation, which inherits the validated indicators and receives the progress of the original hunt in its briefing — the original report stays unchanged, and the resumption goes back through the playbook checkpoint. A report resulting from a resumption displays a link to its parent hunt.
